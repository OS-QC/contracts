...
import { IFlashLoanSimpleReceiver } from "../../protocol-v2-master/contracts/flashloan/interfaces/IFlashLoanSimpleReceiver.sol";
import { ILendingPoolAddressesProvider } from "../../protocol-v2-master/contracts/interfaces/ILendingPoolAddressesProvider.sol";
import { ILendingPool } from "../../protocol-v2-master/contracts/interfaces/ILendingPool.sol";
...
contract AaveFlashLoan is IFlashLoanSimpleReceiver {
    ...
    constructor(
        ...
        dexAddress = _lendingPoolAddressesProvider;
        tokenIn = _tokenIn;
        tokenOut = _tokenOut;
        poolFee = _poolFee;
    ...
    function requestFlashLoanSimple(address asset, uint256 amount) external onlyOwner {
        address receiverAddress = address(this);
        bytes memory params = "";
        uint16 referralCode = 0;

        ILendingPool lendingPool = ILendingPool(ILendingPoolAddressesProvider(dexAddress).getLendingPool());
        lendingPool.flashLoanSimple(receiverAddress, asset, amount, params, referralCode);
    }
    ...
    function executeOperation(
        ...
        // Lógica para usar el préstamo flash
        uint256 amountOwing = amount + premium;
        IERC20(asset).approve(address(dexAddress), amountOwing);
        return true;
    }
