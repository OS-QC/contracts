...
contract ERC2771Forwarder is EIP712, Nonces {
    ...
    function _checkForwardedGas(uint256 gasLeft, ForwardRequestData calldata request) private pure {
        ...
        if (gasLeft < request.gas / 63) {
            ...
            // neither revert or assert consume all gas since Solidity 0.8.27
            // https://docs.soliditylang.org/en/v0.8.27/control-structures.html#panic-via-assert-and-error-via-require
            assembly ("memory-safe") {
                invalid()
            }
        }
